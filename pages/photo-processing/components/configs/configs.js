...
Component({
    ...
    attached: function () {
        ...
        wx.request({
            url: `${getApp().globalData.config.apis.base}/photo-processing/exam/list?appid=${getApp().globalData.config.appid}`,
            success: res => {
                if (res.statusCode !== 200 || !res.data.success) {
                    ...
                    this.setData({
                        ...
                        wx.request({
                            url: `${getApp().globalData.config.apis.base}/photo-processing/config/list?appid=${getApp().globalData.config.appid}&current=${this.current}&ownerErpExamId=${this.data.categories[0].id}`,
                            success: res => {
                                if (res.statusCode !== 200 || !res.data.success) {
                                    wx.hideLoading() // 隐藏 loading
                                    getApp().methods.handleError({ err: res, title: "出错啦", content: res.data.errorMessage ? res.data.errorMessage : '获取规格配置失败', reLaunch: true })
    ...
    methods: {
        ...
        chooseCategory: function (e) {
            ...
            // 请求规格数据
            wx.request({
                url: `${getApp().globalData.config.apis.base}/photo-processing/config/list?appid=${getApp().globalData.config.appid}&current=${this.current}&ownerErpExamId=${e.detail.id}`,
                success: res => {
                    if (res.statusCode !== 200 || !res.data.success) {
                        wx.hideLoading() // 隐藏 loading
                        getApp().methods.handleError({ err: res, title: "出错啦", content: res.data.errorMessage ? res.data.errorMessage : '获取规格配置失败', reLaunch: true })
        ...
        loadMore: function (e) {
            if (this.data.configs.length < this.Total) {
                ...
                wx.showLoading({ title: '获取规格配置', mask: true })
                wx.request({
                    url: `${getApp().globalData.config.apis.base}/photo-processing/config/list?appid=${getApp().globalData.config.appid}&current=${this.current}&ownerErpExamId=${this.data.currentCategory.name}`,
                    success: res => {
                        if (res.statusCode !== 200 || !res.data.success) {
                            wx.hideLoading() // 隐藏 loading
                            getApp().methods.handleError({ err: res, title: "出错啦", content: res.data.errorMessage ? res.data.errorMessage : '获取规格配置失败', reLaunch: true })

...
Page({
    ...
    onLoad: async function (options) {
        ...
        wx.request({
            url: `${getApp().globalData.config.apis.base}/simple-help-event/config/info/${options.id}`,
            success: res => {
                        ...
                        getApp().methods.loginCheck({
                            crmEventFormSID: res.data.data.crmEventFormSid, suffix: { suffix: this.data.suffix, suffixStr: this.data.suffixStr }, remark: `活动表单ID:${res.data.data.crmEventFormSid};简易助力活动;${res.data.data.name};${options.id};`, callback: ({ phone, openid }) => {
                                ...
                                if (typeof options.inviter !== 'undefined') {
                                    ...
                                    wx.request({
                                        url: `${getApp().globalData.config.apis.base}/simple-help-event/log/${options.id}`,
                                        data: { inviter: phone },
                                        success: res => {
                                            wx.hideLoading() // 隐藏 loading
                                            if (res.statusCode !== 200 || !res.data.success) {
    ...
    */
    ...
    onShow: function () {
        ...
        wx.showLoading({ title: '获取邀请记录', mask: true })
        wx.request({
            url: `${getApp().globalData.config.apis.base}/simple-help-event/log/${this.data.config.id}`,

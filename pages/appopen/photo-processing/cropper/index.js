...
Page({
    ...
    input: function (e) {
        if (e.detail.value.trim()) {
            ...
            wx.request({
                url: `${getApp().globalData.config.apis.base}/photo-processing/config/list?appid=${getApp().globalData.config.appid}&current=${this.current}&search=${e.detail.value.trim()}`,
                success: res => {
    ...
    loadMore: function (e) {
        if (this.data.configs.length < this.Total) {
            ...
            wx.showLoading({ title: '获取规格配置', mask: true })
            wx.request({
                url: `${getApp().globalData.config.apis.base}/photo-processing/config/list?appid=${getApp().globalData.config.appid}&current=${this.current}&search=${this.inputValue}`,
                success: res => {
                    if (res.statusCode !== 200 || !res.data.success) {
                        wx.hideLoading(); // 隐藏 loading
                        getApp().methods.handleError({ err: res, title: "出错啦", content: res.data.errorMessage ? res.data.errorMessage : '获取规格配置失败', reLaunch: true });
    ...
    chooseConfig: function (e) {
        ...
        wx.request({
            url: `${getApp().globalData.config.apis.base}/photo-processing/config/info/${e.detail.id}`,
            success: res => {
                if (res.statusCode !== 200 || !res.data.success) {
                    wx.hideLoading(); // 隐藏 loading

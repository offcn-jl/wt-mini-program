  ...
  methods: {
    ...
    push2crm({ phone, crmEventFormSID, suffix, remark }) {
      if (phone && crmEventFormSID) {
        // 推送数据
        wx.request({ url: `https://dc.offcn.com:8443/a.gif?sid=${crmEventFormSID}${remark ? `&remark=${remark}` : ''}`, data: suffix && suffix.suffix ? suffix.suffix : {} })
        // 记录推送日志
        wx.request({ url: `${getApp().globalData.config.apis.base.replace('wechat/mini-program', '')}/crm/push/log?sid=${crmEventFormSID}${remark ? `&remark=${remark}` : ''}`, data: suffix && suffix.suffix ? suffix.suffix : {} })
      }
    },
